...
const postSetDataMode = async (req, res) => {
  ...
  try {
    ...
    const insertSql = `INSERT INTO ${TB_MD} (pump_st, current_level, go_level, mode, start_date, start_time, devices_node_id) VALUES (?, ?, ?, ?, ?, ?, ?)`;
    // const updateSql = `UPDATE ${TB_MD} SET pump_st = ?, current_level = ?, go_level = ?, mode = ?, start_date = ?, start_time = ? WHERE devices_node_id = ?`;
